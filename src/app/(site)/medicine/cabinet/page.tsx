...
const Page = () => {
  ...
  const breadcrumbItems = [
    { title: 'ตู้ยา', link: '/drug/cabinet' },
  ];
  const { toastError } = useToasts();
  const getApi = ApiCall({
    key: ['cabinet'],
    method: 'GET',
    url: `medicine/cabinet?page=${page}&q=${q}&limit=${limit}`,
  })?.get;
  const getApiv = ApiCall({
    key: ['cabinetv'],
    method: 'GET',
    url: `medicine/cabinetview?page=${page}&q=${q}`,
  })?.get;
  ...
  const cols = columns;
  const filterTitle1 = 'สถานีตู้' as string;
  const filterTitle2 = 'ตำแหน่งตู้' as string;
